...
public class UnitTests {
    ...
    public static void main(String[] args){
        ...
        String pattern = RandomPattern.patternFromString(CharRange.LATIN_PRINTABLE, string);
    ...
    private void testWords(CharRange charRange){
        ...
        for (int i = 0; i < NUMBER_OF_WORDS; i++) {
            String word = RandomPattern.randomString(charRange, RussianRoulette.randomInRange(MIN_WORD_LENGTH, MAX_WORD_LENGTH));
    ...
    private void rexTest(CharRange charRange, String string, int N){
        for (int i = 0; i < N; i++) {
            String pattern = RandomPattern.patternFromString(charRange, string);
            System.out.println(pattern);
